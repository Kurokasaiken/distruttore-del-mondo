...
export function makeGraph(params = {}) {
  ...
  const diagonalBiasPct = params.diagonalBiasPct || config.diagonalBiasPct;
  const degree3Pct = params.degree3Pct || config.degree3Pct;
  const degree4Pct = params.degree4Pct || config.degree4Pct;
  const interRegionEdges = params.interRegionEdges ?? config.INTER_REGION_EDGES;
